...
export const heFangJiaoZhongRoles: readonly SmartRoleDefinition[] = [
  ...
  role({ id: "vortox", name: "涡流", officialName: "Vortox", team: "demon", abilityText: "Each night*, choose a player: they die. Townsfolk abilities yield false info. Each day, if no-one is executed, evil wins.", inputKinds: ["player"], possibleOutcomes: ["Each other night, chosen player dies.", "Townsfolk info must be false, even if the Townsfolk is drunk or poisoned; info not from Townsfolk abilities (rule explanations, role/alignment changes) is unaffected.", "If nobody is executed during the day, evil wins; exiling a Traveller does not count as an execution."], stateChanges: ["Target may die."], playerMessageTemplates: ["You chose {seat}."], highRiskNotes: ["False information and no-execution evil win require storyteller confirmation; no auto-win."] }),
  role({ id: "stormcatcher", name: "暴风捕手", officialName: "Storm Catcher", team: "loric", abilityText: "Name a good character. If in play, they can only die by execution, but evil players learn which player it is.", inputKinds: ["role"], setupImpact: ["Name a good character at setup."], possibleOutcomes: ["If in play, that character can only die by execution.", "Evil players learn the player.", "If the named character is not in play, each evil player is woken on the first night and shown that this character is not in play."], stateChanges: ["Stormcaught protection can affect death handling."], playerMessageTemplates: ["The stormcaught character is {role}."], highRiskNotes: ["Loric rule is never placed in the normal seat identity or bluff pool."] })
]
