...
导入向导可在后台先保存“待核对草稿”，但只有 `ScriptDefinition.contentHash === ScriptKnowledgePack.scriptContentHash` 且四项 `coverage` 均为 `ready` 时，剧本才能进入“可开局/可智能配板”列表。角色列表、技能文本或关键顺序变更后必须重新核对；不能继续沿用旧知识包悄悄给出智能建议。项目材料中已经保存的社区/自定义角色说明可作为当前运行口径；没有官方 Wiki 不再单独触发 `needs-review`。明确缺字段、冲突或手动标记待复核时，才阻止智能建议或要求说书人核对。
