...
- 兼容接口模式下，前端可以把 API Key 保存到当前浏览器本地，供后续 AI 请求使用；Key 不会写入 GitHub、归档或响应。
- 后端仍支持从环境变量读取 API Key；共享设备或 VPS 场景优先使用后端环境变量。
...
如果使用“兼容接口”模式，点“保存设置”后，之后的 AI 配板、夜间建议和赛后复盘都会继续使用这份配置。要让它真正调用后端，请把“保存位置”设为 HTTP，并填写本机或 HTTPS 后端地址；普通 HTTP 公网地址不会携带浏览器里的 Key。
...
如果 VPS 没有配置 `BOTC_AI_API_KEY`，仍可以在受信任的本机浏览器里使用已保存的兼容接口配置；共享设备或多人使用时建议只用 VPS 环境变量，避免把 Key 保存到公共浏览器。
