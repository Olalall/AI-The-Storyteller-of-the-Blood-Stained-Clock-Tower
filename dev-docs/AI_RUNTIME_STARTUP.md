...
- GitHub 下载版默认没有 API Key；本机设置页可以把用户自己的 Key 保存到当前浏览器，并只交给 loopback 后端使用。
- 公网 VPS 仍从环境变量读取 API Key；本机浏览器保存的 Key 不会发送给远程后端。
...
6. 点击“保存设置”后，本机运行版会记住地址、模型和 Key；下一次配板、夜间或复盘会继续使用。共享电脑不要保存 Key。
