## 2026-08-26 - 主持台核心界面精简

### 给非开发者看的交付说明
- 本轮属于界面优化，没有修改角色技能、夜间结算、投票规则或 AI 权限。
- 主持台现在只突出“当前要做什么”和“玩家状态”：例如直接显示“继续第3夜”，主按钮也叫“继续第3夜”。
- AI 配板、发身份、计时、切换板子和主持方式收进默认折叠的“更多主持工具”；需要补记昼夜时仍能手动进入对应工作台。
- 顶部只保留本局记录、返回本局和“更多”；新手教学、收尾与复盘收进“更多主持功能”，避免危险的结束操作长期占据主界面。
- 身份尚未领取完时，主任务直接变成“去发身份”；使用实体身份牌的说书人也能选择“已用实体牌”，不会被电子领取流程卡住。
- 玩家详情只显示与该玩家真正相关的记录，不再把别人的行动重复铺进每个玩家详情。
- 手机标题只显示中文板子名，平板和桌面仍保留中英文全名；完整名称同时保留在无障碍标签和鼠标提示里。
- 玩家统计拆成“12人 · 存活12 · 死亡0”和下一行“点卡核对”，不再连成容易误读的一串文字。
- “气球驾驶员”“筑梦师”等较长角色名在手机和平板不再被省略号或状态徽标截断。
- 顶部阶段轨道会自动把当前阶段移到可见区域，并支持方向键、Home、End 查看，夜间和投票状态不会藏在屏幕外。

### Before / After
- Before：主持工具、夜晚、白天、最近记录、结束对局和历史复盘同时铺开；常用与低频、普通与危险操作并列，手机上需要先判断该点哪一个。
- After：默认页面只剩一个当前主任务、玩家状态和一个折叠工具入口；日志只在顶部“本局记录”查看，收尾只在“更多”里进入。

### 验证
- `npm run check` 通过：1827 项测试通过，代码规范、生产构建、PWA 和架构检查通过。
- 完整 Chromium 主持流程 48 项通过，2 项需专门环境的批量脚本/真实 AI 测试按配置跳过。
- 360px、390px 与 iPad 模拟共 12 项通过，3 项不适用场景按配置跳过；覆盖主任务、更多菜单、嵌套新手教学、焦点返回、44px 触控、横向溢出、离线主持、阶段自动定位和 200% 字号。
- 已目视检查 390px 与 iPad 主持台截图，确认中文标题、玩家统计和长角色名均完整显示。

### 风险
- 低频工具比以前多一次“展开/打开”操作，这是主动换取主界面清晰度的取舍。
- 生产包仍提示主 JavaScript 文件偏大，本轮没有做代码分包；不影响功能，但首次加载性能仍有优化空间。
- 自动验收使用 Chromium 模拟尺寸，正式发布前仍建议用真实 Android 与 iPhone/iPad 各走一遍主流程。

## 2026-08-27 - GitHub 首页截图重拍

### 给非开发者看的交付说明
- 本轮属于项目展示优化，没有修改游戏规则、角色技能、夜间结算或 AI 权限。
- GitHub 首页不再把桌面截图压成两列小图；本局总览、AI 配板和夜间工作台改为全宽展示。
- 重新生成 12 人示例局截图，玩家卡、夜间当前角色和手机身份页都能看到角色图标。
- 新增手机主持台和单人身份展示；按维护者反馈，全部 15 张面板截图都直接放在首页上方，不再折叠隐藏。

### Before / After
- Before：桌面截图被缩到半栏，字和按钮偏小；手机截图停在发身份列表，角色图标不明显，看起来像页面缺素材。
- After：核心桌面页面按 1440px 宽度完整展示；手机同时展示带图标的玩家状态和大图身份卡。

### 验证
- 截图脚本会等待角色图片真正加载完成后再拍，缺图时直接失败，不再静默生成无图标截图。
- 所有截图继续由项目示例数据自动生成，不包含 API Key 或私人 VPS 信息。
- 角色原图仍被 `.gitignore` 排除，只提交生成后的项目界面截图。

### 风险
- 截图展示了角色视觉素材，但原始素材没有进入仓库；来源和权利边界继续以 `THIRD_PARTY_NOTICES.md` 为准。
- GitHub 首页图片较多，首次打开会多加载几张 PNG，向下阅读正文的距离也会变长；这是为了让访客无需展开就能看全所有面板。

## 2026-08-26 - 项目内六步新手教学

### 给非开发者看的交付说明
- 开局页新增“新手教学 · 2分钟看懂一局”，不会首次强制弹出，也不会修改当前对局。
- 教学分为主持方式、确认配板、身份交接、夜间主持、白天投票、结束复盘 6 步，每次只显示当前一步。
- 每一步都写清楚“照着怎么点”“看到什么算完成”“最容易误操作什么”，并使用项目里的真实按钮名称。
- 正式开局后，顶部阶段栏仍保留“新手教学”，主持过程中忘记操作时可以重新打开。
- 原有示例对局没有删除，移到教学最后一步作为次要操作；未选择主持方式时不能载入，避免误触。

### Before / After
- Before：开局页只有三条概括流程，用户知道项目大概能做什么，却不知道发身份、结束夜晚、记录处决和保存复盘具体该点哪里。
- After：用户可以按 6 步教学完成一整局操作认知，不用离开项目查 README；关闭或完成后，下次重新打开会从第一步开始。

### 验证
- 全项目门禁通过：1823 项单元/组件测试通过，代码规范、生产构建、PWA 和架构检查通过。
- 完整 Chromium 主持流程回归 48 项通过，2 项可选真实 AI / 批量测试按配置跳过。
- 360px、390px 与 iPad 模拟共 9 项通过，覆盖教学打开、44px 触控目标、横向溢出、关闭后焦点返回和 200% 字号。

### 风险
- 教学解释的是项目操作，不替代《血染钟楼》规则教学和说书人裁定。
- 当前仍主要通过 Chromium 手机和平板尺寸模拟验收，真实 iPhone Safari 与 Android 浏览器需要发布前人工走一遍。

## 2026-08-26 - 首次使用与开局界面减负

### 给非开发者看的交付说明
- 第一次打开不再强迫用户先处理安装提示，直接从“选择主持方式”开始；安装入口收进可选区域，以后再装也不影响使用。
- 开局流程增加“第 1 / 4 步”到“第 4 / 4 步”的短引导，依次完成主持方式、板子与人数、选择方案、最终核对。
- 玩家昵称和经验改为可选折叠项，默认昵称可以直接开局；手机首屏能直接看到“生成配板方案”。
- 配板候选默认只展开第一套，其余方案先显示摘要，减少连续角色卡造成的长页面；AI 没有明确推荐时，不会把第一套伪装成“推荐”。
- 配板确认按钮固定在页面底部，并显示“为什么现在能确认 / 为什么暂时不能确认”；浏览其他候选时不会挡住候选内容。
- 夜间和白天按钮改成更明确的动作名称，例如“确认并停留”“确认并下一位”“准备结束本夜”“确认结束白天”。
- 本轮属于界面优化，没有改变角色规则、夜间结算、玩家隐私或 AI 权限；AI 结果仍只是草稿，最终写入必须由说书人确认。

### Before / After
- Before：首次安装提示挡在开局前，12 个玩家资料输入和多套完整候选占据大量手机页面，关键按钮需要寻找；“确认本项”“确认关闭”等文案不容易判断点完会发生什么。
- After：新用户沿着四步引导完成开局，每屏突出一个主要动作；可选资料默认收起，候选按摘要浏览，确认区持续可见，按钮直接说明会停留、进入下一位还是结束阶段。

### 验证
- 单元/组件测试、代码规范、生产构建、PWA 构建与架构边界检查通过。
- Playwright 浏览器回归覆盖开局、配板、首夜、白天投票、结束归档与 AI 入口。
- 手机/平板模拟覆盖 360px、390px 和 iPad 尺寸，检查首屏主按钮、触控尺寸、横向溢出、200% 字号和离线夜间/投票流程。

### 风险
- 当前是 Chromium 尺寸模拟与自动点击，不等于真实 iPhone Safari 和 Android 浏览器；发布前仍建议各找一台真机走一遍“首次打开 → 配板 → 首夜”。
- 生产包仍有单个 JavaScript 文件偏大的构建警告，本轮未做代码分包；不影响功能，但会影响首次加载速度。

## 2026-08-17 - 手机 / 平板可放心分享版（alpha.3）

### 给非开发者看的交付说明
- 项目现在按 PWA 方式提供 Android、iPhone 和 iPad 主屏幕安装路径，不需要制作 APK；Android/iOS 真机安装仍列在发布前人工验收中。
- 首次打开不再把安装、配板、板子库、示例和魔典选择堆在同一屏：先处理安装或试用，再选实体/电子魔典，最后开始配板。
- 回访用户会直接看到上次使用的主持方式和“开始配板”，不重复阅读新手教程；需要修改时仍可点“更改”。
- Android 没有出现系统安装按钮时会明确指向浏览器菜单；以后也能从“应用设置 → 安装与离线使用”重新找到入口。
- 第一次联网完整打开后，断网仍可重新打开并继续使用配板、夜序、投票、日志和本机复盘；真实 AI 和云端归档仍需联网。
- 开局页增加安装说明；Android 支持浏览器安装按钮，iPhone/iPad 明确提示 Safari 的“共享 → 添加到主屏幕”。
- 新版本准备好后只提示，不会在主持中自动刷新；一局进行中会建议结束后再更新。
- “应用设置”新增当前对局导出与恢复。导入文件先检查内部结构并显示板子、人数和进度，只有再次确认才整局替换；导错后可立即撤销。
- API Key 的说明同步改正：兼容接口配置保存到当前设备的浏览器，不会写进 GitHub。

### Before / After
- Before：手机只能把项目当普通网页打开；首次页并列展示多个入口，不知道先安装、选魔典还是配板；刷新依赖网络；换设备缺少恢复闭环。
- After：首次路径固定为“安装或试用 → 选择魔典 → 开始配板”，回访直接开局；同时可从主屏幕启动、离线重开，并形成“导出 → 预览 → 确认恢复”的换机闭环。

### 验证
- PWA 构建检查确认 Manifest、Service Worker、192/512/maskable 图标和 React 注册接口完整，`/api/` 不进入离线缓存。
- Chromium 390×844 生产构建实测：联网加载后切断网络，刷新仍能进入开局页并显示离线能力边界。
- Chromium 390×844 实测：选择 JSON 后先预览、不立即覆盖；点击确认后才恢复指定对局。
- Chromium 412×915 生产构建实测：断网重开已有对局后，仍能确认一条夜间记录和一轮白天票型。
- 原有手机、分屏、平板和桌面夜间/响应式回归 14 项通过。
- 390×844 实测首次引导与回访精简页：只有一个突出主动作，模式未选时“开始配板”明确禁用。
- 最终候选全项目门禁有 1821 项单元/组件测试通过；Chromium 浏览器回归 48 项通过，2 项可选真实 AI 测试按规则跳过。
- 公开发布审计与架构守门通过；源码和构建配置未写入 API Key 或 VPS 地址。

### 风险
- 本机浏览器数据仍可能被系统或用户清理；PWA 不能替代主动导出备份。
- iPhone/iPad 没有统一的程序化安装按钮，需要走浏览器分享菜单；当前浏览器没有入口时再改用 Safari。
- 当前自动验收使用 Chromium 模拟尺寸；iOS Safari 与 Android 真机安装仍需发布前各做一次人工确认。
- 首次离线包约 6.2 MB（传输压缩后更小）；真实 AI、后端连通测试、云端归档和首次素材下载不能离线运行。

## 2026-08-07 - 角色翻译、首夜顺序与 AI 规则一致性修复

### 给非开发者看的交付说明
- 修复一批角色名、技能文本、夜间提示和 AI 建议中的旧译文或错误规则，重点覆盖半兽人、气球驾驶员、国王、哈迪寂亚、数学家、落难少女、政客和管家。
- 首夜的爪牙/恶魔信息现在按官方夜序插入；疯子会先于恶魔信息处理，罂粟种植者会隐藏邪恶阵营信息，提线木偶不会被错误暴露。
- 同一局出现多个相同角色时，每个座位都会生成独立夜间步骤；半兽人现在会显示必需的玩家目标输入。
- 街头风琴手现在每晚都有“醉酒至下个黄昏 / 保持清醒”二选一；这是玩家本人决定，因此不显示 AI 推荐，也不会自动修改醉酒状态。
- 落难少女首夜改为一张逐个通知爪牙的系统步骤卡，不再高亮落难少女本人；提线木偶会被排除，没有合格爪牙时会明确显示“无需通知”。
- 气球驾驶员现在必须同时记录“本夜展示的玩家”和“本夜按镇民/外来者/爪牙/恶魔哪种类型展示”；下一夜与 AI 只读取紧邻上一夜已经确认的登记，不再从玩家真实角色倒推。
- 月之子白天公开选择玩家时必须登记该玩家当时按善良或邪恶判断；夜间只生成死亡候选，系统和 AI 都不会直接把玩家改成死亡。

### Before / After
- Before：部分导入板子沿用旧规则文案，AI 与夜间步骤可能给出冲突建议；重复角色可能只保留最后一个座位；落难少女通知会错误高亮本人；街头风琴手不会进入夜序。
- After：共用中文技能、AI 规则摘要和夜间步骤采用同一批修正后的规则；群体通知按实际接收者生成；误登记和选择时阵营有明确快照；玩家本人决定不会交给 AI；所有状态变化仍由说书人确认。

### 验证
- 新增角色夜间契约、首夜系统步骤、共用中文技能和复杂角色 AI 摘要回归测试。
- 覆盖罂粟种植者别名、提线木偶隐藏、疯子顺序、重复村夫、半兽人目标、气球驾驶员跨夜条件、国王等号边界和哈迪寂亚逐次结算。
- 覆盖四个含街头风琴手的板子、落难少女全部首夜条目、提线木偶排除和“仅提线木偶时无需通知”。
- 覆盖气球驾驶员登记输入门、紧邻上一夜历史、AI 浏览器/HTTP 契约、月之子白天阵营快照和“死亡候选不自动落盘”。
- 浏览器 smoke 已确认：上一夜登记可展开查看，未选择本夜展示类型时结果按钮不可用，选择后仍需点击“确认本项”。

### 风险
- 风琴手的夜间选择已记录，但闭眼秘密投票仍由说书人手工主持；系统不会自动切换白天投票模式或在黄昏清除醉酒。
- 旧存档或升级前已经创建的夜间草稿没有新增登记字段时，会明确按“未记录/待核对”处理，不会从当前身份补猜；若要获得完整跨夜建议，需要从新建夜晚开始留下确认快照。
- 月之子的死亡仍是候选，必须由说书人在玩家状态面板另行确认；本轮没有把夜间候选与最终死亡记录强绑定。

## 2026-08-02 - Windows 简洁安装教程

### 给非开发者看的交付说明
- 新增一页式安装教程，并在 README 顶部提供入口。
- 普通用户只需下载便携包、完整解压、双击 `启动血染钟楼AI说书人工具.cmd`。
- 便携包根目录的启动、素材安装和教程文件已改为中文名称。
- 明确区分便携包和 GitHub 源码 ZIP，减少“缺少完整便携包”的误操作。

### Before / After
- Before：安装信息位于 README 后部和较长的 Windows 说明中，新用户不容易第一时间找到。
- After：README 打开后即可进入简洁教程，首次素材安装、AI 可选配置和常见错误均有短说明。

### 风险
- 本轮只调整便携包入口文件的中文名称和相关文档，不改变启动逻辑、素材下载逻辑或应用功能。

## 2026-08-02 - 官方与第三方角色素材按需安装

### 给非开发者看的交付说明
- Windows 便捷包现在能在用户确认后安装当前智能板子所需的官方及第三方角色图标，不再只覆盖一小部分官方角色。
- 素材清单覆盖 718 个角色图标，来自 TPI Toolmaker Resources、GStone 与清单记录的社区来源；每个文件都带 SHA-256 校验。
- 基础 GitHub 仓库和便捷包仍不直接夹带二进制素材；拒绝下载时核心功能照常使用，只显示文字占位。

### Before / After
- Before：安装器只覆盖 126 个官方图标，第三方角色仍可能模糊、缺图或依赖外部链接；素材检测还会把 Vite 的 HTML 回退误判为已安装。
- After：718 个角色路径统一投影到本地素材包；首次启动或双击 `Install-Character-Assets.cmd` 即可按需下载，HTML 回退不再计为有效图片。

### 验证
- 素材清单测试确保 718/718 个当前角色路径都有来源、URL 和 SHA-256。
- 安装器会把官方、第三方、已下载和已校验数量写入本机回执。
- 素材状态检测限制为 12 路并发，避免设置页同时发出数百个请求。

### 风险
- 完整素材约 102 MB，首次安装需要联网并等待下载完成。
- 第三方素材仍归各自作者或权利人，不属于本项目 MIT License；若原始来源下线，需要更新来源清单，而不能把失联素材直接提交进公开仓库。
...
### 给非开发者看的交付说明
- GitHub 现在有固定反馈模板：Bug、功能建议、智能板子/规则提醒反馈。
- 模板会主动提醒不要贴 API Key、私人对局数据或大段版权文本，也会要求填写设备、人数、板子、AI 是否启用等关键信息。
- GitHub 已补充 `smart-script` 和 `rules-review` 标签，方便把智能板子/规则提醒类反馈单独归类。
- 这轮是发布治理，不改产品功能、不部署、不调用真实 AI。

### Before / After
- Before：用户只能开空白 Issue，容易漏掉复现步骤、设备尺寸、板子人数或 AI 状态。
- After：不同类型反馈会走不同模板，信息更完整，后续维护更省脑子。
...
### 风险
- 当前仍允许空白 Issue，方便特殊反馈；如果后续垃圾反馈变多，可以再关闭 blank issue 或增加更严格模板。

## 2026-07-27 - 自托管 / VPS 运行手册

### 给非开发者看的交付说明
- 新增一份从本机运行到 VPS 自托管的完整说明：怎么验证、怎么打包、怎么同步、怎么启动、怎么检查、怎么备份和怎么回滚。
- README 里增加了“自托管 / VPS”入口，外部用户不用翻很深的开发文档也能找到部署说明。
- 这轮是文档和发布治理，不实际登录或修改 VPS，不读取或写入任何真实 API Key。

### Before / After
- Before：已有 VPS 边界文档和 AI 启动文档，但缺少一份按步骤操作的总手册。
- After：`SELF_HOSTING_RUNBOOK.md` 把本机、VPS、AI、归档数据、旧 V2.5 共存、备份和回滚串成一条可执行路径。
...
### 风险
- 这只是运行手册，不代表当前已经把最新 commit 同步到 VPS。真正远端部署仍需要单独确认目标、端口、服务托管方式和是否启用真实 AI。
...
## 2026-07-31 - 智能板子复核状态重排

### 给非开发者看的交付说明
- 复核“当前 / 需复核 / AI可用”同时出现造成误解的问题：现在每张板子只突出一个开局门槛，AI建议改为低一级的说明文字。
- “需复核”改成“可开局·需核对”，明确表示这类板子可以使用，但 AI 建议必须由说书人再核对；“暂缓”改成“暂缓开局”。
- 切换板子页不再把全部板子写成“已核对板子”，改为“智能板子”，每张卡直接显示真实质量状态和来源待核对提示。
- 质量看板新增按“全部 / 可直接开局 / 可开局·需核对 / 暂缓开局”筛选，方便集中处理复核队列。

### Before / After
- Before：右侧三个同级胶囊让人分不清“当前使用”“能不能开局”和“AI是否已核验”；切换板子页的标题也会让人误以为所有板子都已经完全核对。
- After：先看开局门槛，再看 AI 建议状态；复核数量可以筛选查看，状态不会因为界面优化而被伪造为 confirmed。

### 验证
- `npm test -- --run src/domain/scripts/quality.test.ts src/features/script-library/ScriptQualityPanel.test.tsx` 通过：2 个测试文件 / 7 项测试。
- `npm run lint` 通过。
- `npm run build` 通过；Vite 仍提示既有的 5.47MB 主 chunk 体积警告。
- `npm run check` 通过：178 个测试文件 / 841 项测试（含 1 个跳过文件、3 项跳过测试），build 与 architecture verification 全部通过。

### 风险
- 当前 117 个智能板子中约 82 个仍是板子级 `needs-review`，这是真实的来源/版本复核状态，不应批量改成 confirmed。
- 本轮只调整质量投影、文案和筛选，不自动修改角色规则、AI建议或权威对局状态。
## 2026-07-31 - 智能板子复核原因可见化

- **解决的问题**：复核数量多时，只看到“待核对”仍不知道具体要核对什么。
- **现在的表现**：每个板子会单独列出来源、人数模板、角色知识、角色调研、板子规则、夜序和模板核验等复核项及数量。
- **复核操作**：进入“可开局·需核对”或“暂缓开局”后，还可以按具体复核项继续筛选板子。
- **边界**：这是维护看板，不会把待核对状态自动改成已确认，也不会替说书人判断规则是否正确。
- **验证**：`npm run check`（178 个测试文件、842 个测试通过）；本地开发服务首页返回 HTTP 200。浏览器自动化通道本轮不可用，未伪称完成视觉截图验收。

## 2026-07-31 - 首批智能板子来源核对

- **解决的问题**：不再要求用户手工逐个核对 82 个板子的来源完整性。
- **完成内容**：对 82 个待复核板子的公开来源 URL 执行 HTTP 拉取和 SHA-256 校验；70 个哈希匹配，已确认来源；3 个哈希不匹配，9 个来源暂不可访问，继续保留待核对。
- **当前状态**：117 个智能板子中 105 个可直接开局，12 个仍显示“可开局·需核对”。
- 这是第一轮复核完成时的历史快照；随后第二轮已处理其中 8 个，当前状态见下方“来源复核第二轮”。
- **边界**：来源哈希通过不等于自动规则引擎通过；技能结果、身份、阵营、死亡和胜负仍由说书人确认。
- **证据**：`dev-docs/script-import-work/batch-03/SOURCE_HASH_AUDIT_2026-07-31.md`。

## 2026-07-31 - 智能板子来源复核第二轮

- **完成内容**：为 Carousel 和 botcscripts 板子找到可直接访问的公开 JSON/API 端点，并完成内容哈希或角色集合核对。
- **结果**：再确认 8 个板子；当前 117 个智能板子中 113 个可直接开局，剩余 4 个为官方 Recommended 板子，因没有可直接校验的 JSON 端点暂保留待核对。
- **边界**：这次确认的是来源和角色集合，不是自动规则结算授权；技能结果仍由说书人确认。

## 2026-07-31 - 智能板子来源复核第三轮

- **完成内容**：通过 TPI 官方 Custom Scripts 页面跳转的 Script Tool，解码 `One in one out`、`A Grimm Chorus`、`Hide & Seek`、`Lunar Eclipse` 的压缩 JSON 载荷，并逐个校验 SHA-256。
- **结果**：4 个板子的载荷哈希均与 pack 声明一致；117 个已注册智能板子现在全部达到来源复核门槛，质量看板不再有板子级“需复核”项。
- **边界**：来源复核不等于自动结算授权。夜间技能结果、身份/阵营/状态变化和胜负仍必须由说书人确认。
- **证据**：`dev-docs/script-import-work/batch-03/SOURCE_HASH_AUDIT_2026-07-31.md`；来源为 [TPI 官方 Custom Scripts 页面](https://bloodontheclocktower.com/pages/custom-scripts)。

## 2026-07-31 - 夜间 AI 质量回归复核

- **完成内容**：重新运行 12 条复杂角色夜间建议回归：赌徒、舞蛇人、方古、麻脸巫婆、普卡、诺-达鲺、红唇女郎、炼金术士、数学家、疯子、洗脑师等。
- **结果**：12/12 通过；建议均保持为“可采用草稿”，不会自动修改身份、阵营、死亡、毒醉或胜负。
- **验证命令**：`npx vitest run src/services/ai/nightSettlementQualityRegression.test.ts --reporter=verbose`。
- **边界**：这是本地规则投影回归，不是外部真实模型质量证明；真实模型仍需配置 API 后单独 smoke。

## 2026-07-31 - 智能板子浏览器抽样回归

- **完成内容**：用真实浏览器点击流抽样验证前 12 个智能板子：切换板子、选择 12 人、生成配板候选、采用草稿、确认座位并进入夜晚工作台。
- **结果**：12/12 通过；每个样本都能生成 12 个座位、进入夜序页面并看到 AI 推荐入口。
- **验证命令**：`$env:BOTC_BULK_SCRIPT_LIMIT='12'; $env:BOTC_BULK_NIGHT_SAMPLE_LIMIT='12'; npm run smoke:smart-scripts:browser`。
- **边界**：这是 12 个板子的浏览器抽样，不等于 117 个板子全部逐个点击验收；全量 smoke 需要更长运行时间。

## 2026-07-31 - 117 个智能板子全量浏览器回归

- **完成内容**：在真实浏览器点击流中遍历全部 117 个智能板子，逐个执行切换板子、12 人配板、采用候选、确认座位并进入夜晚工作台。
- **结果**：117/117 通过，未发现板子选择、配板候选、座位生成或夜晚入口阻塞。
- **验证命令**：`$env:BOTC_BULK_SCRIPT_LIMIT='all'; $env:BOTC_BULK_NIGHT_SAMPLE_LIMIT='12'; npm run smoke:smart-scripts:browser`。
- **边界**：这证明 UI 流程和结构化数据接线可运行，不证明真实模型对每个角色的语义建议都正确；真实模型仍需独立 live smoke。

## 2026-07-31 - 模拟主持与故障降级回归复跑

- **完成内容**：重新运行 12 人主链路、结束归档、7 人小局、15 人大局和缺图标降级场景。
- **结果**：5/5 浏览器用例通过；覆盖配板、夜序、状态确认、投票、归档、复盘入口和缺少角色素材时继续开局。
- **验证命令**：`npx playwright test tests/e2e/manual-click-smoke.spec.ts tests/e2e/game-end-prototype.spec.ts tests/e2e/smoke-hosting-scenarios.spec.ts --reporter=line`。
- **边界**：这是本地模拟主持验证，不等于真实 VPS 重启/断网演练。

## 2026-07-31 - 后端 runtime 预发布 smoke 复跑

- **完成内容**：重新执行 server 单测、后端 runtime 构建和临时端口 HTTP smoke。
- **结果**：server 测试 48/48 通过；runtime 可启动、健康检查、归档写入和 fake 复盘均通过，AI 默认保持关闭。
- **验证命令**：`npm run test:server`、`npm run smoke:backend`。
- **边界**：这证明本地 runtime 可运行，不等于已经完成 VPS 重启、端口冲突和远端归档持久化演练。

## 2026-07-31 - Windows 便捷启动包

### 给非开发者看的交付说明
- 新增 `npm run package:portable`，生成一个可上传 GitHub Release 的 Windows ZIP。
- 使用者解压后双击 `scripts/portable/Start-Storyteller.cmd`，第一次启动可填写 AI 接入地址、模型名和 API Key，也可以跳过 AI。
- AI 配置只写入本机 `.env`；便捷包不包含密钥、个人归档、V2.5 或第三方二进制素材。
- 便捷包和 VPS 打包分开：普通使用者走便捷包，VPS 仍走 `package:vps` / `sync:vps`。

### Before / After
- Before：GitHub 使用者需要自己安装依赖、读命令和手动设置环境变量。
- After：安装 Node.js 20 LTS 后，解压并双击启动脚本即可；AI 可在首次启动时配置。

### 风险与边界
- 第一版仍需要 Node.js 20 LTS；暂不引入 Electron/Tauri，避免桌面壳层扩大架构。
- 启动脚本会在本机保存 `.env`，不要把该文件发给别人或提交到 GitHub。
...
## 2026-08-07 - 驱魔人 / 魔鬼代言人跨夜目标修复

### 给非开发者看的交付说明
- 修复驱魔人和魔鬼代言人的夜间目标输入：现在会显示上一夜由说书人确认的目标，并禁用该座位，避免连续两夜误选同一人。
- 如果上一夜记录后来被更正，已经创建的本夜页面再次打开时也会读取更正后的最终版本，不继续沿用旧缓存。
- 历史缺失、角色曾经更换或旧存档无法确认上一目标时，AI 只会提示补资料，不会猜一个可直接采用的结果。
- 所有结果仍是候选记录；身份、状态、技能结果和权威游戏状态仍由说书人确认。

### Before / After
- Before：这两个角色没有稳定的单人目标输入，也不会可靠阻止重复选择；AI 可能只读技能文案而看不到上一目标。
- After：页面、魔典座位点击和 AI 请求使用同一份最终历史；上一目标不可重复，历史不完整时明确停在人工核对。

### 验证
- 跨夜历史、纠错链、旧角色别名、换角隔离和 AI 降级定向测试：37/37 通过。
- 真实浏览器夜间角色流程：5/5 通过。
- `npm run check`：见本轮交付记录。

### 风险与下一步
- 普卡、沙巴洛斯、阎罗、珀等角色还需要“中毒归因 / 死亡来源 / 动态目标数”专用结构，不能复用本轮的一目标禁选逻辑直接自动化。

## 2026-08-07 - 普卡 / 沙巴洛斯 / 阎罗 / 珀跨夜候选

### 给非开发者看的交付说明
- 普卡会显示最近一次由说书人明确确认、且尚未结算的成功中毒目标；死亡、恢复健康和新中毒仍分别只是候选。
- 沙巴洛斯会把上一夜选中过且当前死亡的玩家列为可反刍对象，同时仍要求本夜选择两名目标。
- 阎罗会显示滚动延迟死亡候选；《子规泣鸣》的首夜目标只在第三夜额外出现，不套用到同名但文案不同的其他板子。
- 珀现在能明确记录“选择无人进行蓄力”，下一次实际行动会要求恰好三名目标；被阻止行动不会被误算成空刀。
- 历史不完整时，页面提示人工核对，AI 返回需要补充信息，不从当前状态或模型常识倒推。

### Before / After
- Before：不同板子的输入定义会让珀显示 0、1 或 2 个固定目标，沙巴洛斯有时只能选一人；普卡和阎罗只有技能文案，没有可靠的跨夜候选。
- After：四个角色使用同一套说书人确认时间线投影，但各自拥有独立规则契约；死亡、中毒、恢复和复活不会自动写入状态。

### 验证与边界
- 浏览器覆盖旧毒候选、反刍候选、阎罗延迟候选和珀三目标流程。
- AI 只接收最小化角色历史，历史座位只能生成待确认状态草稿。
- 《九泉颂歌》阎罗的资料仍存在文字冲突，本轮按该板展示的滚动上一夜目标能力处理，没有擅自加入《子规泣鸣》的第三夜特殊条款。
## 2026-08-07 - 夜间历史/AI 候选一次性收口

### 给非开发者看的交付说明

- 气球驾驶员现在会记住上一夜说书人明确登记的角色类型。健康时不能连续给同一类型；如果气球驾驶员中毒或醉酒，则允许同类型错误信息。
- 月之子会区分白天记录是“生效”“未生效”还是缺少选择当刻阵营；白天已记录未生效、或目标当前已死亡时，夜里不会再生成新的死亡候选。
- Professor 等夜间每局一次能力如果此前已有确认使用记录，再次出现在夜晚队列时会标成“已用过，人工核对”，AI 不再推荐再次发动。
- 历史修正入口会按角色结果限制目标数量，例如珀蓄力/无行动不能带目标、珀三杀必须三名、沙巴洛斯必须两名、普卡无新毒不能写成错误目标数。

### Before / After

- Before：部分历史记录只靠技能文案提醒，AI 可能在缺历史时继续给建议，或者修正器写出不符合角色契约的目标数。
- After：关键历史被结构化成 ready / clear / missing；缺历史时 AI 停在补资料，同类型气球和已用过的限次能力不会被当成可直接采用结果。

### 验证

- `npx vitest run src/features/game-session/state/nightHistoricalContext.test.ts server/ai/nightSettlementProvider.test.ts src/features/night-workbench/state/wakeInputGating.test.ts`
- `npm run build`

### 风险与边界

- 保护类角色如僧侣、旅店老板、水手、和平主义者仍只作为说书人确认候选，不自动取消死亡。
- Gambler 继续走现有目标真实身份 + 猜测角色比对，不需要跨夜历史。
## 2026-08-07 - 夜晚目标契约一次性补齐
### 给非开发者看的交付说明
- 修复一批角色夜晚技能“该选几个人/能不能选无人/是否必须换类型”的记录口径，重点覆盖贵族、哈迪寂亚、典狱长、饕餮、万钧之力、知事分子、沙巴洛斯、普卡、阎罗、珀等角色。
- 现在说书人夜晚面板会按角色显示正确目标范围：例如贵族和哈迪寂亚是三名目标，典狱长是 1-3 名，饕餮/走私犯可记录任意数量，万钧之力只接受一名或三名。
- AI 夜晚建议现在按同一套目标契约判断是否缺输入，不会因为旧的“所有多人目标都等于 2 人”规则误判。

### Before / After
- Before：导入包里部分角色 inputKinds 是 `none` 或泛化的 `players`，夜晚面板可能没有目标格、只能选 2 人，或者 AI 建议把还没满足契约的草稿当作可确认。
- After：角色目录会正规化常见导入缺口，并用能力文本保守推断“一名/两名/三名/任意/至少/至多”目标契约；状态变化仍只是候选草稿，最终由说书人确认。

### 验证
- 新增/更新 `role-night-contracts`、`wakeInputGating`、`firstNightSystemSteps` 覆盖 input 元数据、目标范围、1 或 3 目标分支、0 或 1 目标分支和 AI 缺输入判断。
- 只读审计后剩余 29 组 `inputKinds: none` 主要是数字信息、关键词、条件触发、方向选择等非玩家目标契约，未强行转换为玩家目标。

### 风险
- 文本推断是保守规则，只解决明确写出玩家/目标数量的导入缺口；“左/右方向”“关键词”“多个角色猜测”等更细输入类型仍需要后续单独建模。
- 本轮不自动结算任何死亡、中毒、醉酒、阵营或身份变化；只让夜晚记录和 AI 建议有正确的候选输入边界。
## 2026-08-13 - 新项目角色文案、夜间 AI 审查与 API 配置持续生效

### 给非开发者看的交付说明
- 修复并统一多项角色中文技能、说书人提示和 AI 使用的能力文本，重点处理复杂角色、夜间顺序和高风险结果。
- 角色名称、技能说明、夜间提示和 AI 输入统一；智能板子、角色资料和 AI 边界在 README 与界面中写清楚。
- AI API 设置现在可以保存到当前设备的浏览器；保存后配板、夜间建议和赛后复盘会继续使用。
- API Key 不会进入 GitHub、对局归档、模型提示词或返回内容；只有本机或 HTTPS 后端会携带浏览器保存的 Key。
- 修复 VPS 启动脚本语法问题；默认保持本机绑定，公网访问仍需受保护的反向代理。

### Before / After
- Before：部分角色文案、夜序和 AI 输入口径不一致；API Key 保存后不再用于正式请求；VPS 计划任务可能启动失败。
- After：角色和 AI 使用同一套规则资料；保存一次后后续 AI 请求继续使用本机配置；VPS 新版本可以由计划任务正常启动。

### 验证与风险
- 已通过全量单元测试、构建、Lint、架构检查、后端 smoke、公开发布审计和关键浏览器 smoke。
- 复杂角色结果和 AI 建议仍然只能作为草稿，最终由说书人确认。
- 浏览器本地存储不是密码保险箱，不建议在公共电脑上保存 API Key。
- 社区/自定义角色仍按项目内保存的来源资料使用，不把缺少官方 Wiki 的角色伪装成官方规则。

## 2026-08-17 - 手机浏览器模拟点击复审与误触修复

### 给非开发者看的交付说明
- 用 Playwright Chromium 在 360px Android、390px Android 和 768px iPad 尺寸下反复模拟手指点击，覆盖首次打开、回访、设置、配板、离线夜间和白天投票。
- 修复快速双击“先试用”后第二下穿透到下一页、替用户误选主持方式的问题。
- 修复手机设置页底栏压住高级参数的问题；打开设置也不再自动检查 718 个角色图标，只有点击“重新检测”才发请求。
- 未开局时隐藏无关的阶段、记录和收尾导航；“先浏览板子”不能再绕过主持方式选择。
- 昵称输入框提升到至少 44px；200% 大字体下顶部阶段栏不再撑破页面；未开始阶段文字和键盘焦点更清楚。
- iPhone/iPad 会提示从浏览器分享菜单添加到主屏幕；当前浏览器没有入口时再建议改用 Safari，安装按钮增加重复触发保护。

### Before / After
- Before：双击可能误选模式；设置高级参数可能点到“恢复默认”；大字体会横向溢出；打开设置会产生数百次素材探测。
- After：关键动作必须由用户明确选择，手机点击区和大字体回流有自动测试，设置页不会误触或自动制造大量请求。

### 验证与边界
- `npm run check`：278 个测试文件通过、1 个跳过；1744 项测试通过、3 项可选 live 测试跳过。
- `npm run test:e2e`：47 项通过、2 项可选 live 测试跳过。
- `npm run test:e2e:pwa`：3 项通过。
- `npm run test:e2e:mobile-audit`：9 项通过、3 项按设备范围跳过。
- iPhone/iPad 目前是 Chromium 的触控、尺寸和 UA 模拟；真实 Safari 的“共享 → 添加到主屏幕”仍需真机发布前走一次。

## 2026-08-17 - API Key 与移动端验收口径修正（仅文档）

### 给非开发者看的交付说明
- 统一 README、安装教程、产品愿景和公开发布边界：在页面中保存的 API Key 会留在当前设备的当前浏览器 `localStorage`，以后打开同一站点可继续使用。
- 明确“保存在玩家本机”不等于上传 GitHub，也不等于保存到 VPS/服务器；换设备、换浏览器或清除站点数据后需要重新填写。
- 明确浏览器本地存储不是密码保险箱，公共电脑、共享浏览器或不可信扩展环境不建议保存真实 Key。
- 把“手机人工实测”“iOS 已验证”等容易误解的说法收紧为实际证据：目前自动检查使用 Playwright Chromium 的尺寸、触控和 UA 模拟，Android/iOS 真机仍待发布前验收。

### Before / After
- Before：README 已写 Key 会保存在浏览器，但产品愿景和公开发布边界仍写“前端不保存”；移动端说明也容易让人误以为已经完成 WebKit 真机测试。
- After：所有当前文档都区分浏览器本地保存、服务器环境变量和 GitHub 公开内容，并把自动模拟与真机验收明确分开。

### 验证与边界
- 口径依据现有设置持久化代码和测试：配置会写入 `botc-copilot-ai-settings-v1`，后续配板、夜间建议和复盘请求会读取；普通 HTTP 公网后端不会接收浏览器保存的 Key。
- 设置保存、配板请求、夜间建议和赛后复盘相关定向测试：4 个测试文件、21 项测试通过。
- 本项只修改文档，没有修改程序行为，也没有把当前其他未提交代码预先写成“已通过全部测试”。全量检查仍应在最终发布提交上重跑。

## 2026-08-17 - 存档安全、手机可用性与公网 AI 止血修复

### 给非开发者看的交付说明
- 第二个浏览器标签页不再只是显示“只读”文字：它现在会真正停止对局操作和自动保存，避免两个窗口互相覆盖记录。原窗口关闭后，接管窗口会先读取最后保存的数据。
- AI 连通测试不再把“用户填写的接入地址”和“VPS 自己的 API Key”拼成一套配置；浏览器自带配置缺任何一项都会拒绝调用，堵住服务器 Key 被发往错误地址的路径。
- 对局导入、手机安全区、窄屏提示和脚本搜索的本轮变化见同日最终验证记录；未通过最终门禁前不提前标记为已发布。

### Before / After
- Before：第二标签页仍能操作并保存；live-test 可以逐字段回退到服务器配置。
- After：只读标签页的工作区由浏览器原生 `inert` 禁用且停止持久化；live-test 只允许“整套浏览器配置”或“整套服务器配置”，不再混用。

### 风险与边界
- 默认本机/自用模式仍不适合匿名公开；同日后续新增的显式公开模式会拒绝 `/api/archives*`、`/api/recovery/*` 和使用 VPS Key 的 AI，并增加限流与请求体上限。
- 隐藏 VPS 地址不是安全措施；即使启用公开模式，也不能把 runtime 端口直接暴露到公网，仍应由 HTTPS 反向代理转发。

### 最终验证
- `npm run check` 通过：281 个测试文件通过、1 个跳过；1769 项测试通过、3 项可选 live 测试跳过；生产构建、PWA 产物和架构门禁通过。
- `npm run test:e2e`：48 项桌面/响应式浏览器流程通过，2 项可选 live 测试跳过。
- `npm run test:e2e:pwa`：3 项生产 PWA 离线与备份恢复流程通过。
- 移动端 Chromium 审查：9 项通过、3 项按设备范围跳过，覆盖 360/390px Android、768px iPad 模拟和 200% 字号。
- `npm run audit:public` 与 `git diff --check` 通过；仍有 3 条已有 lint warning 和大包体提醒，不阻塞本轮。


## 2026-08-17 - 公开 VPS 访问边界

### 给非开发者看的说明
- 项目新增显式的“公开分享模式”。默认仍是原来的本机/自用模式，不会因为更新代码就自动变成匿名公网服务。
- 公开模式只匿名开放网页、PWA 和健康检查；服务器归档、半局恢复数据和使用 VPS Key 的 AI 不向匿名用户开放。
- 普通用户仍可在自己的手机、平板或电脑浏览器里保存对局、查看本机归档、离线主持并导出 JSON，不需要 VPS 云端存档才能使用核心功能。
- 用户自带 Key 的 AI 是可选能力，只允许连接部署者白名单中的 HTTPS provider，并受服务端频率和请求大小限制。
- 隐藏 VPS 地址不是安全措施；GitHub 不写地址只能避免主动曝光，不能代替后端拒绝规则、反向代理和防火墙。

### Before / After
- Before：只要 runtime 被匿名转发，访问者就可能读取服务器归档/恢复数据；AI live-test 也缺少完整的公网白名单、请求大小和频率边界。
- After：显式公网模式会在后端直接拒绝私有归档/恢复接口，隐藏并禁用 VPS AI 配置；BYOK 必须整套提交，只能访问白名单中的 HTTPS 主机，并拒绝重定向。

### 验证与风险
- 公网安全定向回归 4 个测试文件、68 项通过；后端目录共 17 个测试文件通过、1 个跳过，139 项通过、3 项可选 live 测试跳过。
- 全项目门禁通过：284 个测试文件通过、1 个跳过，1821 项通过、3 项可选 live 测试跳过；生产前端构建、PWA 产物和架构守门通过。
- 生产后端 smoke 同时验证默认本机模式和显式公网模式：本机归档照常可用；公网健康检查返回成功，服务器归档返回 403，AI 设置显示关闭且无服务器 Key。
- PWA 浏览器回归 3 项通过，确认离线重开、备份恢复和离线夜晚/白天记录没有被本轮后端边界影响。
- Windows 便携包已生成并解压复核：34.04 MB，内置 Node.js 24.18.0，不含 `.env`、Key、依赖目录或角色二进制素材。
- 示例只使用占位域名，没有记录真实 VPS 地址或任何 API Key。
- 当前限流只按 runtime 看到的连接地址计算，不信任可伪造的转发头；反向代理后的所有用户可能共享每分钟 10 次 AI 请求限额。这是偏保守的安全边界，上线 BYOK 前应按实际代理拓扑做压力验收。
- 本轮没有部署到 VPS，也没有进行真实公网、真实域名、HTTPS 或真机验收；上线仍需单独授权并在目标环境复测。

## 2026-08-27 - VPS 角色图补全与状态卡显示

### 给非开发者看的交付说明
- 本轮属于界面修复和部署治理，没有修改角色技能、夜间顺序、投票规则或 AI 决策权限。
- 主持台的玩家状态卡新增小型角色圆盘；VPS 有素材时显示真实角色图，没有素材或单张图片损坏时自动显示角色简称，不再出现破图。
- 玩家状态卡在手机上由三列改为两列，并改成左图右文；平板由六列改为四列，长角色名和状态不再挤在窄卡片里。
- 私人 VPS 由服务器统一保存 718 张角色素材，访问网页的人不需要自己安装或下载素材包。
- GitHub 开源包仍不包含角色图片二进制文件，只包含来源清单和可选安装脚本，避免把授权边界不明确的素材直接公开分发。

### Before / After
- Before：VPS 没有角色素材，手机玩家状态区硬塞三列，玩家卡片窄；图片路径失效时也缺少明确回退。
- After：VPS 直接提供角色图，手机状态区使用两列横向卡片；缺图环境仍稳定显示简称，不影响开局和主持。

### 验证
- `npm run check` 通过：1828 项测试通过，代码规范、生产构建、PWA 和架构检查通过。
- Android 390px 本地与 VPS 公网自动点击、图片解码和截图检查通过，角色圆盘不挤压长角色名，页面没有横向溢出。
- 公网健康接口、角色图片和旧 V2.5 页面均返回 HTTP 200；服务器角色素材数量为 718。
- 公开发布审计通过；VPS 包中的 6 张图片均为应用 PWA 图标，不包含角色素材。

### 风险
- 第三方角色素材依赖其来源继续可用；VPS 已保存当前副本，所以日常使用不受临时外网波动影响。
- 公网慢速加载时圆盘会先显示底色，图片解码后再出现；实测 12 个示例角色均成功解码为 400×400 图像。
- 首次在其他电脑离线运行 GitHub 下载版时仍只显示角色简称，除非用户主动运行素材安装脚本。

## 2026-08-27 - 配板、发身份与收尾页移动端减负

### 给非开发者看的交付说明

- 修复平板配板方案错位：提示文字不再占掉一个方案位置，三个候选会从同一行整齐开始。
- 手机发身份时，“当前领取”和“打开单人展示”移到座位列表前面；不再需要先划过全部玩家才能完成当前领取。
- 手机发身份的进度区、当前身份卡和座位卡更紧凑，但按钮仍保留至少 44px 的触控高度。
- 手机结束与复盘页压缩标题、胜方选项、保存摘要和步骤间距，让“选择结果”和“保存本局”更早出现；重置仍保留单独确认。

### Before / After

- Before：768px 平板的第三个配板方案被挤到下一行；手机发身份的当前操作排在 12 个座位之后；收尾页首屏被大封面和大卡片占满。
- After：配板候选按正常三列排列；领取身份先看到当前操作、再核对座位；收尾页优先显示胜方与保存动作。

### 风险与边界

- 本轮只调整响应式排版和操作顺序，不修改配板内容、身份数据、胜负判断、归档或重置逻辑。
- 发身份和收尾仍由说书人明确点击确认，不会因为页面变紧凑而自动发送、自动保存或自动重置。
