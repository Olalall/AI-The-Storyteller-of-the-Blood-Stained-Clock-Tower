...
## 2026-08-10 - 新项目角色文案与夜间 AI 审查
### 给非开发者看的交付说明
- 本轮审查的是新项目 `AI-The-Storyteller-of-the-Blood-Stained-Clock-Tower`，不是已经停用的 V2.5 项目。
- 修正并统一了多项角色中文技能、说书人提示和 AI 使用的能力文本，重点处理傻瓜、教父、心上人、方古、酒鬼、驱魔人，以及此前仍显示英文技能的角色。
- 夜间流程现在会使用同一套规范化角色 ID 和中文能力文本；导入脚本中的官方角色夜序会按官方夜表校正，避免角色有夜间技能却被队列漏掉或排错。
- 教父只有在“外来者白天被处决并实际死亡”时才提供额外击杀候选；方古第一次用该能力杀死外来者后，后续夜晚不再错误建议再次转化。所有结果仍必须由说书人确认。

### Before / After
- Before：部分角色的文字会误导说书人，某些导入脚本的角色 ID 变体可能让夜间队列找不到角色，AI 也可能依据旧英文文本给出错误候选。
- After：角色名称、技能说明、夜间提示和 AI 输入统一；高风险角色会显示历史条件和人工复核提示，不会自动写入死亡、身份或阵营结果。

### 验证与风险
- 已通过全量单元测试、构建、Lint、架构检查；完整端到端套件单 worker 运行结果为 44 项通过、2 项按环境条件跳过，完整游戏结束用例另行重复 3 次通过。
- 批量智能脚本浏览器冒烟在本地限定时间内未结束，因此该批量长流程仍未标记为完全通过；未发现新的断言失败输出。
- 本轮没有新增真实 AI 服务调用，也没有把 AI 变成自动裁决器；若后续接入真实 API，仍需单独做网络、超时、费用和错误恢复验收。

## 2026-08-10 - 智能板子角色官方资料核对与使用说明

### 给非开发者看的交付说明

- 全量盘点了当前新项目的 117 个智能板子、3133 条角色记录和 718 个去重角色 ID，并建立了 [智能板子角色与官方资料核对报告](SMART_SCRIPT_ROLE_WIKI_AUDIT_2026-08-10.md)。
- 把“官方角色资料基线”“社区脚本角色”“作者自定义角色”分开记录，不再把所有 `confirmed` 都误解成“官方 Wiki 已逐条核对”。
- 修正了杂技演员的官方阵营和选择目标；官方同名自定义角色不会再被全局官方文案覆盖。
- 增加 Loric 分类并修正风暴捕手的特殊角色分类；它不会进入普通座位配板或恶魔伪装。
- 修正驱魔人、麻脸巫婆、月之子和僵怖的夜间提示/候选边界：恶魔只是不因自身能力醒来；目标角色已在场时麻脸巫婆无效；月之子本夜醉毒时不生成死亡候选；僵怖会根据白天死亡记录决定是否行动。
- 继续修正角色 ID 别名和夜序显示：`cult_leader`、`al-hadikhia` 现在会按统一 ID 排序；英文来源占位提醒在运行时改成中文说明。
- 发现 Acrobat 的社区同名技能存在“邻近玩家”变体后，改为只修正明确匹配官方版本的记录，并同步修正受影响的十三行、寄梦他乡配板模板；没有强行覆盖社区变体。
- README、Windows 教程、脚本库、配板页和夜间结果区现在直接说明：智能板子是本地知识包，AI 只是候选/草稿层，最终仍由说书人确认。

### Before / After

- Before：README 写“100 个智能板子”，界面也没有解释智能板子到底在哪使用；官方角色、社区角色和自定义角色混在同一个“已核验”概念里，个别角色会被错误套用同 ID 的官方文案。
- After：数量、使用位置和 AI 边界写清楚；官方可确认问题先修复，无法由官方 Wiki 证明的社区角色单独列出，不再伪装成官方核对结果。

### 验证与边界

- 新增角色文案、阵营分类、月之子和僵怖历史条件的单元测试。
- 官方资料依据：`release.botc.app/resources/data/roles.json`，以及 Acrobat、Storm Catcher、Loric、Exorcist、Moonchild、Pit-Hag、Zombuul 官方 Wiki 页面。
- 仍未声称 718 个角色全部完成官方 Wiki 逐条人工语义复核；564 个社区/自定义 ID 需要按原作者来源继续分批确认。

## 2026-08-10 - 社区/自定义角色来源降级与 AI 研究占位清洗

### 给非开发者看的交付说明

- 以前：角色资料只要通过导入结构检查，就可能统一显示为“已确认”；AI 也会收到一些“按来源技能处理”之类的通用占位语句，看起来像具体规则。
- 现在：运行时会把不在官方角色目录、或明确标为 Custom 的角色标成“待复核”，并在配板 AI、夜间 AI 中提醒“这是社区/自定义来源，不是官方 Wiki 核准规则”。
- 这不是删除角色，也不会禁止开局；它只是让说书人知道哪些内容需要自己核对。
- AI 研究摘要会过滤通用占位语句，避免模型把提示语误当成角色实际结果；真正的技能文字仍按板子来源提供。

### 验证与边界

- 当前运行时统计：117 个板子、3133 条角色记录；2254 条来自官方角色目录 ID，879 条属于社区/自定义或未知 ID。来源标记保留在内部，但不再因为没有官方 Wiki 页面而显示“需要核对”。
- 项目内已经整理的角色说明作为当前使用口径；这不是把模型临时生成的回答当成规则，而是使用仓库中保存的角色资料和研究摘要。
- AI 仍然只生成草稿和提醒，身份、阵营、死亡、中毒、醉酒、复活和胜负必须由说书人确认。
## 2026-08-10 - 开源使用入口与 API 配置口径优化

- 这轮是开源使用体验和安全边界优化，不改变角色技能裁定，也不让 AI 直接写入权威状态。
- API 设置页现在明确区分“后端长期配置”和“临时兼容接口测试”：页面不会保存 API Key，长期使用要配置后端 `.env`。
- 空白新局入口增加三步开局说明，帮助第一次使用的人从选板子走到首夜确认。
- 自定义 JSON 导入入口改成明确的规划状态，避免用户点击一个实际不可用的按钮。
- VPS 启动脚本默认只绑定本机；公网访问应通过带认证的反向代理，只有明确完成外围保护时才显式传入 `-AllowPublicBind`。
- 项目版本标为 `0.1.0-alpha.1`，避免 GitHub 用户把当前预览版误认为正式稳定版。
## 2026-08-10 - GitHub 下载版本机保存 AI 配置

### 给非开发者看的交付说明
- GitHub 和便携包默认不包含任何 API Key。
- 用户在自己电脑上填写 API 地址、模型和 API Key 后，点击保存，下一次打开仍会保留。
- 本机保存的配置会用于后续配板建议、夜间建议和赛后复盘草稿，不再只是做一次连通测试。
- 本机模式只把 Key 发给本机 loopback 后端；连接公网 VPS 时不会把这份本机 Key 发给远程服务器。

### Before / After
- Before：页面可以保存地址和模型，但 API Key 只存在当前页面；刷新或重新打开后需要重新输入，而且后续 AI 请求不会使用页面输入的 Key。
- After：GitHub 下载者可以在自己的电脑上保存并持续使用自己的 Key；VPS 模式仍由后端 `.env` 管理。

### 风险
- 浏览器本地保存不是密码保险箱，共享电脑或不可信浏览器环境不应保存 Key；需要更换或撤销 Key 时，应在模型服务商后台轮换旧 Key。
- AI 仍然只生成候选和草稿，不会自动确认角色结果、修改权威状态或判定胜负。

### 验证
- 设置服务回归测试覆盖本机 API Key 的保存、重新读取和恢复默认。
- AI 配板、夜间和复盘 HTTP 适配器覆盖本机配置注入；服务端只在 loopback 请求中接受客户端配置。
