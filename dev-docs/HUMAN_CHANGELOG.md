...
## 2026-08-10 - 保存 AI 配置后持续用于正式请求

### 给非开发者看的交付说明
- 修复了“输入 API KEY 只能测试一次，保存后正式功能不用”的问题。
- 现在在 AI API 设置页点“保存设置”后，重新打开页面仍能看到已保存配置；AI 配板、夜间建议和赛后复盘会继续使用这份配置。
- API KEY 只保存在当前浏览器本地，不会进入 GitHub、源码、归档或后端返回结果；正式请求仍通过本机/HTTPS 后端代理发出。

### Before / After
- Before：API KEY 只存在设置页临时状态；真实连通测试结束后，后续功能无法使用这把 Key。
- After：保存后的兼容接口配置会随每次正式 AI 请求传给后端代理；后端使用它访问模型，浏览器不会直接访问模型服务。

### 风险与边界
- 浏览器本地保存不是密码保险箱；共享电脑、公共浏览器或浏览器被恶意脚本控制时，仍可能暴露 Key。共享/VPS 场景建议改用后端环境变量。
- 只有本机或 HTTPS 后端会携带浏览器保存的 Key；普通 HTTP 公网地址会自动不携带并回退到后端环境变量/本地草稿。
- AI 仍只生成草稿和建议，不会自动确认配板、改玩家状态、写时间线或推进夜序。

### 验证
- 前端设置服务、配板 HTTP、夜间 HTTP、复盘 HTTP、后端 AI 代理和归档复盘路由均增加了持久化配置回归测试。
## 2026-08-13 - alpha.2：本机 API 配置持续生效与 VPS 启动修复

### 给非开发者看的交付说明
- AI API 设置现在可以保存到当前设备的浏览器中；下次打开后，配板、夜间建议和赛后复盘会继续使用这份配置。
- API Key 不会进入 GitHub、对局归档、模型提示词或返回内容；发送到后端前会限制为本机地址或 HTTPS 地址。
- 修复 VPS 启动脚本中的语法问题，避免计划任务启动后立即失败。
- 发布 Windows 便携包，普通用户不需要安装 Node.js 或开发工具。

### Before / After
- Before：API Key 只能临时测试，保存后后续 AI 功能不会继续使用；VPS 计划任务可能启动失败。
- After：保存一次后，后续 AI 请求继续使用本机配置；VPS 新版本可以由计划任务正常启动。

### 验证
- 前后端 AI 配置、配板、夜间建议、赛后复盘和 HTTP 代理回归测试通过。
- 完整构建、架构检查、后端 smoke 和公开发布审计通过后生成便携包。

### 风险
- 浏览器本地存储不是密码保险箱，不建议在公共电脑上保存 API Key。
- 这是 alpha 预发布版；复杂角色结果和 AI 建议仍然只能作为草稿，最终由说书人确认。
