...
> 后续变更说明（2026-08-10）：GitHub 下载版新增“本机保存 API Key”路径。原文中“前端不保存 Key”描述的是本审计完成时的旧行为；当前实现只允许 loopback runtime 接收浏览器本机配置，公网 runtime 仍使用后端 `.env`，且 Key 不进入响应、归档、日志或 GitHub。
