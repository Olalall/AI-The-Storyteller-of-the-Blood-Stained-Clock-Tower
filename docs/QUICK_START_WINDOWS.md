...
随后可以在启动器里填写接入地址、模型名称和 API Key；这些长期配置会写入解压目录的 `.env`，供本机后端之后持续使用。也可以在应用内“AI API 设置”中填写并保存，GitHub 下载版会把它保存在当前浏览器的本机设置里，并用于之后的配板、夜间和复盘请求。共享电脑不建议保存 Key；也可以跳过 AI，跳过后记录、夜序、投票、日志和归档仍可使用。

## 第一次开局怎么走

1. 点击“开始新局”，选择智能板子和玩家人数。
2. 点击“开始配板”，从候选里查看角色组合、人数构成和恶魔伪装。
3. 可选点击“AI推荐”查看排序和风险提示；点击“采用为草稿”只会填入草稿。
4. 说书人手动调整后点击“确认配板”，再发送身份并进入首夜。
5. 夜间工作台按板子夜序逐项唤醒角色。夜间的“AI推荐”只生成当前结果草稿，选择结果后还要点击“确认本项”。

“可直接开局”表示本地模板、夜序和结构化资料已经纳入项目使用口径；复杂结果仍要按魔典由说书人确认。

配置只写入解压目录内的 `.env`，不会进入 GitHub，也不会显示在页面上。不要把 `.env` 发给别人、上传 GitHub 或截图公开。

如果把服务部署到 VPS，不要直接开放 runtime 端口；让同机的带登录保护反向代理转发到默认的 `127.0.0.1`。只有明确完成防火墙和代理保护时，才需要在启动脚本中使用 `-AllowPublicBind`。
